...
def strandard_attention(
    ...
    **attrs,
    ...
    """
    assert query.dim() == 2 and key.dim() == 2 and value.dim() == 2 and output.dim() == 2
    ...
    **attrs,
    ...
    for start_n_k in range(0, N, TILE_N_K):
        ...
        for start_m in range(0, N, TILE_M):
            ...
            scores = tile_q @ tile_k.T / torch.sqrt(torch.tensor(D, dtype=torch.float32))
            ...
            cur_sum = per_sum * torch.exp(per_max-cur_max) + torch.sum(torch.exp(scores-cur_max), dim=-1)[:, None]

            per_output = output[start_m:start_m+TILE_M, :]
            cur_output = per_output * per_sum / cur_sum * torch.exp(per_max-cur_max) + (torch.exp(scores-cur_max) / cur_sum) @ tile_v
